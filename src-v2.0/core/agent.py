...
class LiveSoftware:
    ...
    def add_method(self, description):
        ...
        for _ in range(20):
            response = client.chat([message], model_parameters=config.model_providers[config.default_provider],temperature=0.5)
            alpha = 3
            beta = 3
            try:
            ...
            if content["operation"] == "query":
                ...
                for i in range(alpha):
                    ...
                    message = LLMMessage(role="user", content=f"Code structure: {self.state_manager.get_structure_str()} \n"                 
                                        + f"Random_token:{num}\n")
                    response = tmp_code_client.chat([message], model_parameters=config.model_providers[config.default_provider],temperature=0.5)
                    try:
                ...
                while(flag == False):
                    ...
                    response = tmp_test_client.chat([message], model_parameters=config.model_providers[config.default_provider],temperature=0.5)
                    try:
                        content_i = json.loads(response.content)
                    except json.JSONDecodeError:
                        flag = False
                        continue
                    if "args" in content_i:
                        args = content_i["args"]
                    else:
                        flag = False
                    test = args
    ...
    def request(self, requirement):
        ...
        for _ in range(10):
            response = self.request_client.chat([message], model_parameters=config.model_providers[config.default_provider],temperature=0.00001)
            try:
                content = json.loads(response.content)
            except json.JSONDecodeError:
